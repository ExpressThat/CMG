...
- Explicit `screenshot` and `screenshotPage` actions can write PNG or JPEG artifacts with `type=` and `quality=`. They can apply temporary artifact-only CSS with `style=` or `stylePath=`; `screenshotPage` can also write clipped artifacts with `clipX=`, `clipY=`, `clipWidth=`, and `clipHeight=`. GIF recorder frames always use CMG's internal PNG capture path so virtual pointer compositing and frame encoding stay consistent.
