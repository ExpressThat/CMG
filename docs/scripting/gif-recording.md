...
- Explicit `screenshot` and `screenshotPage` actions can write PNG or JPEG artifacts with `type=` and `quality=`. They can apply temporary artifact-only CSS with `style=` or `stylePath=`, and artifact-only masks with `mask=` and `maskColor=`. `screenshotPage` can also write clipped artifacts with `clipX=`, `clipY=`, `clipWidth=`, and `clipHeight=`. GIF recorder frames always use CMG's internal PNG capture path so virtual pointer compositing and frame encoding stay consistent.
...
- Screenshot artifact masks do not mask GIF frames. If the viewer should see a redaction or highlight in the GIF itself, change the page with `evaluate`, `highlight`, `caption`, or normal page actions before recording that frame.
