...
- File, fixture, PDF, API, network, worker, coverage, page-error, clock, browser context, accessibility, and storage actions do not move the virtual pointer. Wrap them in `step`, `caption`, or `gif` blocks when their result should be visible to a GIF viewer.
