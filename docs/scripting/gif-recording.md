...
- Structural actions such as `import`, `macro`, `call`, `return`, `if`, `elseif`, `else`, `for`, `repeat`, `while`, `foreach`, `foreachSelector`, `break`, and `continue` do not move the virtual pointer by themselves. Pointer-aware actions inside those blocks still use the same virtual pointer movement, pointer events, hover behavior, drag ghosts, and captions as top-level actions.
