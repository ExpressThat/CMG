...
The default narration path participates in retry, passing-test, and age retention. A custom narration path under the GIF artifact directory is also cleaned with its GIF family. CMG does not follow an old timeline to delete a path outside that directory.
...
Use this file when reports, CI artifacts, or agent feedback need machine-readable timing without parsing the GIF binary. Timeline schema version `2` adds nested/repeated runtime step spans and failure bookmarks. JSON run reports include a `gifMetadata` array with timeline, narration, alt-text, description, and artifact inspection data, while each visual step has a `gifEvidence` array containing exact frame/time boundaries. Retained artifacts also receive parseable `GIF_REPRODUCE` stdout and JSON/HTML reproduction commands that preserve browser, port, project, source, test selection, and recording boundary. HTML reports show the animated artifact with authored alt text, link narration and visual start frames, and pin the final diagnostic failure frame as a static thumbnail. JUnit reports add `cmg.gif.path`, `cmg.gif.narrationPath`, `cmg.gif.altText`, and `cmg.gif.description` properties when available; failed tests with an inspectable GIF also include `cmg.gif.failureFrameIndex`.
