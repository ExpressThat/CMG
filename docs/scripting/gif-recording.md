...
- File, fixture, init-script, runtime tag injection, PDF, API, explicit wait, navigation wait, dialog, network environment, network mock, worker, coverage, page-error, clock, browser context, accessibility, element-state assertion, and storage actions such as `localStorage`, `sessionStorage`, and `cookie` do not move the virtual pointer. Wrap them in `step`, `caption`, or `gif` blocks when their result should be visible to a GIF viewer.
