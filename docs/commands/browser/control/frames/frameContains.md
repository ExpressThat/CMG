# `browser control frames frameContains`

Asserts contained text inside an iframe element using the `frameContains` script action.

```powershell
cmg browser control frames frameContains "<frameSelector>" "<selector>" "<text>" [options]
```
...
```text
PASS 001 frameContains #frame #status Saved
FRAME 001 frameContains
```
...
```powershell
cmg browser control frames frameContains "#checkoutFrame" "#status" "Saved"
```
