...
Options and failure behavior match [`waitForRequest`](waitForRequest.md), including `--match contains|exact|regex` and `--ignore-case` for URL matching. Header filters match response headers.
