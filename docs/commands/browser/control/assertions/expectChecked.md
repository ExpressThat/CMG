# `browser control assertions expectChecked`

Runs the scripting `expectChecked` action once from the command line.

```powershell
cmg browser control assertions expectChecked "<selector>" [--expected <true|false>] [--timeout <milliseconds>]
```
...
```powershell
cmg browser control assertions expectChecked "#agree" --expected false
```
