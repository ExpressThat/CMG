# `browser control assertions expectNotText`

Provider-style alias for [`expectNoText`](expectNoText.md).

```powershell
cmg browser control assertions expectNotText "<selector>" "<expected>" [--timeout <ms>] [--match <mode>] [--ignore-case]
```
...
```text
PASS 001 expectNotText #status Error
```
...
```powershell
cmg browser control assertions expectNotText "#status" "Error"
```
