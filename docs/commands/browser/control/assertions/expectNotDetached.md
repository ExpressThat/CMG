# `browser control assertions expectNotDetached`

Runs the scripting `expectNotDetached` action once from the command line.

```powershell
cmg browser control assertions expectNotDetached "<selector>" [--timeout <milliseconds>]
```
...
```text
PASS 001 expectNotDetached #save
...
```powershell
cmg browser control assertions expectNotDetached "#save"
```
