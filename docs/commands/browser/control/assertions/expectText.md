# `browser control assertions expectText`

Runs the scripting `expectText` action once from the command line.

```powershell
cmg browser control assertions expectText "<selector>" "<expected>" [--timeout <ms>] [--match <mode>] [--ignore-case]
```
...
```text
PASS 001 expectText #status Ready
```
...
```powershell
cmg browser control assertions expectText "#status" "Ready" --timeout 5000
```
