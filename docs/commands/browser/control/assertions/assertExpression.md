# `browser control assertions assertExpression`

Runs the scripting `assertExpression` action once from the command line.

```powershell
cmg browser control assertions assertExpression "<expression>" [--equals <value>] [--contains <text>] [--timeout <ms>]
```
...
```text
PASS 001 assertExpression window.appReady
...
```powershell
cmg browser control assertions assertExpression "document.body.innerText" --contains "Ready"
```
