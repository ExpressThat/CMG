# `browser control assertions expectEval`

Runs the scripting `expectEval` action once from the command line.

```powershell
cmg browser control assertions expectEval "<expression>" [--equals <value>] [--contains <text>] [--timeout <ms>]
```
...
```powershell
cmg browser control assertions expectEval "document.title" --equals "Checkout"
cmg browser control assertions expectEval "window.appReady" --timeout 5000
```
