...
- Selector actions support the same rich locators as `cmg run`, including `text=`, `textExact=`, `role=`, `role=role|name`, `label=`, `labelExact=`, `testid=`, `testId=`, `data-testid=`, `placeholder=`, `placeholderExact=`, `alt=`, `altExact=`, `title=`, `titleExact=`, `xpath=`, `first=`, `last=`, `nth=selector|index`, `has=selector|childSelector`, `hasNot=selector|childSelector`, `hasText=selector|text`, `hasNotText=selector|text`, and `visible=`. Pointer-aware actions resolve the locator to a temporary element marker before moving the virtual pointer.
