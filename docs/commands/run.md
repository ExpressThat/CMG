...
Control-flow and reuse actions `within`, `if`, `elseif`, `else`, `switch`, `case`, `default`, `for`, `repeat`, `while`, `until`, `doWhile`, `doUntil`, `retry`, `toPass`, `foreach`, `foreachSelector`, `break`, `continue`, `try`, `catch`, `finally`, `macro`, `call`, and `return` are available inside tests, hooks, steps, and GIF blocks. They can be nested, and pointer-aware actions inside them use the same virtual pointer behavior as top-level actions.
