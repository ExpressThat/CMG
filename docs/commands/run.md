...
Reports and traces include per-test status, output, and per-step diagnostics so agents can explain why a run failed. JSON `gifMetadata` includes nullable `narrationPath` and `stillPdfPath`; HTML links both reviewer artifacts; JUnit emits `cmg.gif.narrationPath` and `cmg.gif.stillPdfPath`. Completed `download` and `waitForDownload` outputs are validated against the filesystem and exposed as JSON `downloadArtifacts[]` entries with `path`, `name`, and `sizeBytes`, HTML download links, and JUnit `cmg.download.path`/`cmg.download.sizeBytes` properties. Missing files are omitted. Download paths remain excluded from automatic GIF captions.
