...
Requested narration files emit `GIF_NARRATION <absolute-path>`. Runner declarations are `gifNarrationSidecar=`, `gifAltText=`, and `gifDescription=`; suite values cascade and test values override them. Root/project `gifSettings.narrationSidecar`, `altText`, and `description` provide coarse defaults, with CLI and DSL declarations remaining more specific.
