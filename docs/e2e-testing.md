...
- Direct script execution with GIF and trace output, plus stdin script validation that does not require a launched browser.
...
- Runner pre-browser validation failures for invalid config JSON, invalid config field types, invalid shard values, invalid browser ports, and unmatched script paths.
