...
- `147-run-config.cmgscript`: Demonstrates `cmg run --config` defaults with variables, selection, project matrices, reports, traces, workers, and retries.
- `run-config.example.json`: Example JSON config for `cmg run --config`, `--project`, and `--workers`.
