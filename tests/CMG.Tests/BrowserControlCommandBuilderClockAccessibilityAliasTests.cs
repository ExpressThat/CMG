...
public sealed class BrowserControlCommandBuilderClockAccessibilityAliasTests
{
    [Theory]
    [InlineData("install --now 1700000000000", "clock now=\"1700000000000\"")]
    [InlineData("restoreClock", "restoreClock")]
    public void ClockAliasCommands_MapToScriptActions(string commandTail, string expectedScript)
    {
        var handler = new CapturingBrowserControlCommandHandler();
        var exitCode = BuildRoot(handler).Parse($"control clock {commandTail}").Invoke();
    ...
    [Theory]
    [InlineData("snapshot #dialog", "accessibilitySnapshot \"#dialog\"")]
    [InlineData("accessibilitySnapshot #dialog", "accessibilitySnapshot \"#dialog\"")]
    [InlineData("expect --role button --name Save", "expectAccessible role=\"button\" name=\"Save\"")]
    [InlineData("expectAccessible --role button --name Save", "expectAccessible role=\"button\" name=\"Save\"")]
    public void AccessibilityAliasCommands_MapToScriptActions(string commandTail, string expectedScript)
    {
        var handler = new CapturingBrowserControlCommandHandler();
        var exitCode = BuildRoot(handler).Parse($"control accessibility {commandTail}").Invoke();
