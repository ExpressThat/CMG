...
public sealed class BrowserControlCommandBuilderNetworkTests
{
    [Theory]
    [InlineData("waitForResponse /api --header \"Content-Type: json\"", "waitForResponse \"/api\" header=\"Content-Type: json\"")]
    [InlineData("waitForRequest /api --header-name Authorization --header-value Bearer", "waitForRequest \"/api\" headerName=\"Authorization\" headerValue=\"Bearer\"")]
    public void NetworkWaitCommands_MapHeaderFilters(string commandTail, string expectedScript)
    {
