...
public sealed partial class CmgVisualSegmentExecutor
{
    ...
    public CmgTestResult Run(CmgTestCase test, string remoteDebuggingUrl, CmgRunOptions options, int attempt)
    {
        ...
        foreach (var action in CmgVariables.FromRunOptions(options).Concat(test.Actions))
        {
            ...
            {
                var flush = RunLines(pending, pendingLineMap, remoteDebuggingUrl, gif: null, timeouts, baseUrl, options.GifQuality, options.PointerMotion, options.ClickPulse, options.HoldAfterActionMilliseconds, options.HoldOnFailureMilliseconds);
                ...
                if (!TryGifQualityFor(action, out var blockQuality, out error) ||
                    ...
                    !RunActions(action.Children, remoteDebuggingUrl, gif, timeouts, baseUrl, blockQuality, blockMotion, blockPulse, blockHold, blockFailureHold, output, steps, out error))
                {
                    return Fail(test, output, error, gifs, steps);
                }
        ...
        var final = RunLines(pending, pendingLineMap, remoteDebuggingUrl, commandGif, timeouts, baseUrl, options.GifQuality, options.PointerMotion, options.ClickPulse, options.HoldAfterActionMilliseconds, options.HoldOnFailureMilliseconds);
