...
- Add GIF metadata to JSON reports: quality, fps, frame count, duration, dimensions, file size.
...
- Add artifact size warnings in runner output. CLI: `--gif-warn-size <size>`.
...
- Add max duration guard with a clear failure reason. CLI: `--gif-max-duration <duration>`.
